...
async function getEvent(id: string) {
    try {
        ...
        const url = isUUID
            ? `${process.env.API_BASE_URL}/api/events/${id}`
            : `${process.env.API_BASE_URL}/api/events/slug/${id}`;
...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
    ...
    const image = event?.cover_image ?? "https://krownpass.com/og/opengraph.svg";
    return {
        title: `${title} – KrownPass`,
        description: event?.description ?? "View this event on KrownPass",
        openGraph: {
            title,
    ...
    return (
        <html>
            <head>
                ...
                <meta property="al:ios:url" content={`krown://event/${id}`} />
                <meta property="al:ios:app_store_id" content={APP_STORE_ID} />
                <meta property="al:ios:app_name" content="Krown" />
                <meta property="al:android:url" content={`krown://event/${id}`} />
